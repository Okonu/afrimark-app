...
trait DebtorsAnalytics
{
    public function getDebtorsAnalytics(): array
    {
        ...
        // Current invoices
        $currentQuery = Invoice::where('business_id', $business->id)
            ->where('due_amount', '>', 0)
        ...
        // Overdue invoices
        $overdueQuery = Invoice::where('business_id', $business->id)
            ->where('due_amount', '>', 0)
            ->where('days_overdue', '>', 0);
        ...
        foreach ($statuses as $status) {
            $query = DB::table('business_debtor as bd')
                ...
                ->where('bd.business_id', $business->id)
